...
export const EvervaultCard = ({
  ...
  useEffect(() => {
    const str = artists ? generateArtistString(artists, 1500) : generateRandomString(1500);
    setRandomString(str);
  }, [artists]);
...
const fillers = "·•◦∘○●◉◎⊙⊚";
export const generateArtistString = (artists: string[], length: number) => {
  let result = "";
  const shuffled = [...artists].sort(() => Math.random() - 0.5);
  let idx = 0;
  while (result.length < length) {
    const artist = shuffled[idx % shuffled.length];
    // Add some random filler between names
    const fillerCount = Math.floor(Math.random() * 4) + 1;
    for (let f = 0; f < fillerCount; f++) {
      result += fillers.charAt(Math.floor(Math.random() * fillers.length));
    }
    result += artist;
    idx++;
  }
